...
// Supply Chain Intelligence: Fetching Origin Sources
$search = $_GET['search'] ?? '';
...
<html lang="en">
<?php 
$pageTitle = 'Suppliers Ledger Registry';
...
<body>
            ...
            <div class="inventory-view animate-fade-in">
                ...
                <div class="view-header">
                    <div class="header-content">
                        <h1 class="view-title">Supplier Ledger</h1>
                        <p class="view-subtitle">Management of institutional vendor profiles and supply sources</p>
                    </div>
                    ...
                    <div class="search-engine">
                        <div class="search-input-wrapper">
                            ...
                            <form method="GET" action="" style="flex: 1;">
                                <input type="text" name="search" class="search-input" placeholder="Search by company, contact or email..." value="<?= htmlspecialchars($search) ?>" autocomplete="off">
                            </form>
                        </div>
                    </div>
                    <div class="stats-badge">
                        <span class="count-label">Vendor Count:</span>
                        <span class="count-value"><?= count($suppliers) ?> Sources</span>
                    </div>
                </div>
                                    ...
                                    <tr class="no-data">
                                        <td colspan="6">
                                            <div class="empty-state">
                                                ...
                                                <p>No supplier records founded in the procurement ledger.</p>
                                                <a href="add_supplier.php" class="btn btn-outline btn-sm">Initialize First Vendor</a>
                                            </div>
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($suppliers as $s): ?>
                                        <tr class="supplier-row">
                                            <td class="id-col">#<?= str_pad($s['id'], 3, '0', STR_PAD_LEFT) ?></td>
                                            <td>
                                                <div class="identity-cell">
                                                    <span class="s-name"><?= htmlspecialchars($s['name']) ?></span>
                                                    <span class="s-date">Registered: <?= date('M d, Y', strtotime($s['created_at'])) ?></span>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="authority-cell">
                                                    <span class="contact-name"><?= htmlspecialchars($s['contact_person'] ?: 'N/A') ?></span>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="contact-details-cell">
                                                    ...
                                                    <span class="s-email"><?= htmlspecialchars($s['email'] ?: '-') ?></span>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="address-cell">
                                                    <span class="s-address"><?= htmlspecialchars($s['address'] ?: 'No physical address records') ?></span>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="maintain-actions">
                                                    <a href="edit_supplier.php?id=<?= $s['id'] ?>" class="m-btn edit" title="Modify Source">
                                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                                                            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                                                            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                                                        </svg>
                                                    </a>
                                                    <a href="delete_supplier.php?id=<?= $s['id'] ?>" class="m-btn delete" onclick="return confirm('Remove vendor permanently?')" title="Archive">
                                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
    ...
    .supplier-row { transition: 0.2s; }
    .supplier-row:hover { background: rgba(255, 255, 255, 0.02); }
    ...
    .s-name { color: var(--text-primary); font-weight: 700; font-size: 0.95rem; }
    .s-date { color: var(--text-dim); font-size: 0.75rem; }

    .authority-cell { font-weight: 700; color: var(--text-primary); font-size: 0.9rem; }

    .contact-details-cell { display: flex; flex-direction: column; gap: 0.15rem; }
    .s-phone { color: var(--text-primary); font-weight: 600; font-size: 0.9rem; display: flex; align-items: center; gap: 0.4rem; }
    .s-email { color: var(--accent-primary); font-size: 0.75rem; font-weight: 600; }

    .address-cell { color: var(--text-dim); font-size: 0.85rem; line-height: 1.4; max-width: 250px; }
    .s-address { display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
